...
function SignWellIntegration() {
  ...
  const [loaTemplateId, setLoaTemplateId] = useState('');
  const [saved,      setSaved]      = useState(false);
  const [saving,     setSaving]     = useState(false);
  const [testing,    setTesting]    = useState(false);
  const [testMsg,    setTestMsg]    = useState('');
  ...
  useEffect(() => {
    Promise.all([
      ...
      supabase.from('pricing_settings').select('value').eq('key','signwell_loa_template_id').single().then(({data})=>{ if(data?.value) setLoaTemplateId(data.value); });
      setLoading(false);
    });
  }, []);
  ...
  async function save() {
    ...
    await supabase.from('pricing_settings').upsert({ key: 'signwell_loa_template_id', value: loaTemplateId, description: 'SignWell template ID for Letter of Authorization (number porting)' }, { onConflict: 'key' });
    ...
    <div style={{ background: 'white', border: '1px solid #e5e7eb', borderRadius: 8, padding: 20, marginBottom: 20 }}>
      ...
      {loading ? <div style={{ fontSize: 11, color: '#9ca3af' }}>Loading...</div> : (
        <>
          ...
          <label style={{ display: 'block', fontSize: 11, fontWeight: 600, color: '#374151', marginBottom: 4 }}>
            Letter of Authorization (LOA) — Template ID
          </label>
          <input
            type="text"
            value={loaTemplateId}
            onChange={e => setLoaTemplateId(e.target.value)}
            placeholder="SignWell template ID for the LOA signature page"
            style={{ width: '100%', padding: '7px 9px', border: '1px solid #d1d5db', borderRadius: 5, fontSize: 12, outline: 'none', fontFamily: 'DM Mono, monospace', marginBottom: 4 }}
          />
          <div style={{ fontSize: 9, color: '#9ca3af', marginBottom: 10 }}>
            Create a template in SignWell with the LOA signature page, then paste the template ID here.
          </div>
