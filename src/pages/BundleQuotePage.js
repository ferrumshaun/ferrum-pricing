...
export default function BundleQuotePage() {
  ...
  async function connectDeal(deal) {
    ...
    try {
      ...
      setHubMsg(`✓ Connected: ${full.deal.dealname}`); setHubResults([]);
    } catch (e) { setHubMsg('✗ ' + e.message); }
    setHubLoading(false);
  }
  ...
  return (
    <div style={{ display:'flex', height:'100%', overflow:'hidden' }}>
      ...
      <div style={{ width:360, flexShrink:0, background:'white', borderRight:'1px solid #e5e7eb', overflowY:'auto', padding:'12px 14px' }}>
        ...
        {/* HubSpot bar */}
        <div style={{ marginBottom:10, padding:'7px 10px', background:hubDealId?'#fff7ed':'#f8fafc', border:`1px solid ${hubDealId?'#fed7aa':'#e5e7eb'}`, borderRadius:5 }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
            <div style={{ display:'flex', alignItems:'center', gap:5 }}>
              <div style={{ width:14, height:14, background:'#ff7a59', borderRadius:2, display:'flex', alignItems:'center', justifyContent:'center' }}>
                <span style={{ color:'white', fontSize:8, fontWeight:700 }}>H</span>
              </div>
              <span style={{ fontSize:10, color:hubDealId?'#c2410c':'#6b7280' }}>{hubDealId ? hubDealName || `Deal #${hubDealId}` : 'Not connected'}</span>
            </div>
            <button onClick={() => { setHubModal(true); setHubSearch(recipientBiz||''); setHubMsg(''); setHubResults([]); }}
              style={{ fontSize:9, padding:'2px 7px', background:'#0f1e3c', color:'white', border:'none', borderRadius:3, cursor:'pointer', fontWeight:600 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
            ...
            <div style={{ background:'white', borderRadius:6, border:'1px solid #e5e7eb', padding:11 }}>
              ...
              <textarea value={dealDescription} onChange={e=>setDealDescription(e.target.value)} rows={3}
                placeholder="Client context, scope decisions, pricing rationale..."
        ...
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.45)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:300 }}>
          <div style={{ background:'white', borderRadius:10, padding:24, width:500, maxHeight:'75vh', display:'flex', flexDirection:'column', boxShadow:'0 8px 32px rgba(0,0,0,0.18)' }}>
            ...
            <div style={{ flex:1, overflowY:'auto' }}>
              {!hubResults.length && !hubMsg && <div style={{ textAlign:'center', padding:20, color:'#9ca3af', fontSize:11 }}>Search for an open deal above</div>}
              {hubResults.map(d=>(
