...
export default function Layout() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
      {/* Top bar */}
      <div style={{ background: '#0f1e3c', padding: '0 16px', display: 'flex', alignItems: 'center', gap: 16, height: 48, flexShrink: 0 }}>
        {/* Logo */}
        ...
        <div style={{ position: 'relative' }}>
          ...
          {menuOpen && (
            <div style={{ position: 'absolute', right: 0, top: '100%', marginTop: 4, background: 'white', border: '1px solid #e5e7eb', borderRadius: 6, padding: '4px 0', minWidth: 160, zIndex: 100, boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}>
              ...
              <button onClick={handleSignOut} style={{ width: '100%', textAlign: 'left', padding: '6px 12px', background: 'none', border: 'none', fontSize: 12, color: '#374151' }}>
                Sign out
              </button>
            </div>
          )}
